...
for step in [
    ["scp", "-o", "StrictHostKeyChecking=no", "-i", KEY,
     "deploy/_remote_final.sh", HOST + ":/tmp/_remote_final.sh"],
    ["ssh", "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes", "-o", "ConnectTimeout=10",
     "-i", KEY, HOST, "bash /tmp/_remote_final.sh"],
]:
    print("=" * 30)
    r = subprocess.run(step, capture_output=True, text=True, timeout=240, encoding="utf-8", errors="replace")
