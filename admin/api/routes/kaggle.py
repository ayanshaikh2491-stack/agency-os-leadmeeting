...
async def list_tools():
    from admin.tools.kaggle_tools import KAGGLE_TOOLS
    return {"tools": KAGGLE_TOOLS, "count": len(KAGGLE_TOOLS)}


@router.get("/status")
async def kaggle_status():
    from admin.tools.kaggle_tools import _check_kaggle, _get_kaggle_creds
    ...
    return {
        ...
        "models": ["FLUX (images)", "CogVideoX (videos)", "Stable Diffusion"],
    }
...
async def video_ad(body: VideoAdRequest):
    from admin.tools.kaggle_tools import generate_video_ad
