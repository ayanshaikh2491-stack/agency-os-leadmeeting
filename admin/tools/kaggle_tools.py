...
# -- Fix PyTorch for Kaggle P100 (sm_60 needs older PyTorch) --
import subprocess, sys, json, os, traceback

try:
    _gpu_raw = subprocess.check_output(
        ["nvidia-smi", "--query-gpu=name,compute_cap", "--format=csv,noheader"],
        text=True, timeout=10
    ).strip().lower()
    print(f"GPU raw: {_gpu_raw}")
    if "p100" in _gpu_raw or "sm_60" in _gpu_raw:
...
if torch.cuda.is_available() and torch.cuda.device_count() > 0:
    try:
        ...
        if cap[0] >= 8:
            ...
            print(f"Ampere+ (sm_{cap[0]}{cap[1]}) -- bfloat16")
        elif cap[0] >= 7:
            DEVICE = "cuda"
            DTYPE = torch.bfloat16 if cap[1] >= 5 else torch.float16
            print(f"Turing (sm_{cap[0]}{cap[1]}) -- {DTYPE}")
        elif cap[0] >= 6:
            DEVICE = "cuda"
            DTYPE = torch.float16
            print(f"Pascal (sm_{cap[0]}{cap[1]}) -- float16")
        else:
            print(f"GPU too old (sm_{cap[0]}{cap[1]}) -- CPU fallback")
    except Exception as e:
...
try:
    ...
    if "p100" in _gpu_raw or "sm_60" in _gpu_raw:
        ...
        print(json.dumps({{"status": "skipped", "reason": "P100 (sm_60) not supported by CogVideo"}}))
        sys.exit(0)
except Exception as e:
    print(f"GPU check: {e}")
...
if cap[0] < 7:
    print(json.dumps({{
        "status": "skipped",
        "reason": f"GPU sm_{cap[0]}{cap[1]} too old for CogVideo (needs sm_70+)",
        "prompt": "{prompt}"
    }}))
    sys.exit(0)
