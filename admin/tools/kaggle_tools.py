...
# --- P100 (sm_60) check — disable CUDA, CogVideo can't run on CPU ---
import subprocess, sys, json, os, traceback
...
try:
    ...
    if "p100" in out or "sm_6.0" in out or "sm_60" in out:
        P100_MODE = True
        print("P100 detected — CogVideo needs sm_70+ GPU. Video gen skipped.")
        os.environ["CUDA_VISIBLE_DEVICES"] = ""
except Exception as e:
    print(f"GPU check: {{e}}")
...
if P100_MODE:
    print(json.dumps({{
        "status": "skipped",
        "reason": "P100 GPU (sm_60) not supported by CogVideo — needs T4/L4/A100 (sm_70+)",
        "prompt": "{prompt}"
    }}))
    sys.exit(0)
...
if torch.cuda.is_available() and torch.cuda.device_count() > 0:
    try:
        ...
        if cap[0] >= 7:
            ...
            print("GPU too old for CogVideo. Exiting.")
            print(json.dumps({{
                "status": "skipped",
                "reason": f"GPU {{gpu_name}} sm_{{cap[0]}}{{cap[1]}} not supported",
                "prompt": "{prompt}"
            }}))
            sys.exit(0)
    except Exception as e:
    ...
    print(f"No GPU available. CogVideo requires GPU (T4+). Exiting.")
    print(json.dumps({{
        "status": "skipped",
        "reason": "No GPU available. CogVideo requires GPU (T4/L4/A100)",
        "prompt": "{prompt}"
    }}))
    sys.exit(0)
