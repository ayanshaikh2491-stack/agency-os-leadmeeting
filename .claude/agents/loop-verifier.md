...
- Project test/lint commands (`npm test`, `npm run lint` — see AGENTS.md)
- Allowed file scope (if specified by the loop)

## Checklist (all must pass for APPROVE)

1. **Scope**: Only relevant files changed; no denylist paths (`.env`, `.env.*`, `auth/`, `payments/`, `secrets/`, `credentials/`, infra configs); no unrelated edits.
